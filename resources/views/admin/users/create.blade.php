...
<div class="bg-white p-6 rounded-xl shadow max-w-xl">
    <div class="flex items-start justify-between gap-3 mb-5">
        <div>
            <h2 class="font-bold text-lg text-gray-800">Tambah Akun</h2>
            <p class="text-sm text-gray-500">Buat akses login untuk pegawai atau administrator.</p>
        </div>
        <a href="{{ route('admin.users.index') }}" class="px-4 py-2 rounded border text-sm font-semibold hover:bg-gray-50">Kembali</a>
    </div>

    @if($errors->any())
        <div class="bg-red-100 text-red-800 px-4 py-3 rounded mb-4">{{ $errors->first() }}</div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4">
        ...
        <div>
            ...
            <select name="role" class="w-full p-2 border rounded mt-1">
                <option value="user" @selected(old('role', 'user') === 'user')>Pegawai</option>
